...
export default class WebpackDevServer {
  ...
  async start(
    ...
    debug(`Mode: ${options?.integratedProxy ? 'Integrated Proxy' : 'Dual Server'}`)
    debug(`Host: ${host}, Port: ${defaultPort}, Nimbu Port: ${nimbuPort}`)
    ...
    if (options?.integratedProxy) {
      ...
      serverConfig = {
        ...createDevServerConfig(null, ['localhost', '.localhost', ...(urls.lanUrlForConfig ?? [])], options),
        host,
        port,
      }
    } else {
    ...
    debug('Server config:', JSON.stringify(serverConfig, null, 2))
    ...
    try {
      ...
      debug('Error starting DevServer:', error)
      throw error
    }
  }
